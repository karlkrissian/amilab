...
def ImplementFunctionWrap( parsefunc, functioncount=1):
  ...
  if returntypest=="void":
    if parsefunc.name in config.available_operators.keys():
      if len(parsefunc.args)>0:
        if parsefunc.name=='[]':
          res += '  (*this->_objectptr->GetObj()) [ '.format(parsefunc.name)
          res += AddParameters(parsefunc)+"];\n";
        else:
          res += '  (*this->_objectptr->GetObj()) {0} '.format(parsefunc.name)
          res += AddParameters(parsefunc)+";\n";
      else:
...
def WrapFunction(funcname,include_file,inputfile):
  ...
  if dh.found:
    ...
    if parse_function.IsTemplate(funcname):
      shutil.copyfile("wrap_templatefunction.h.in",header_filename)
    else:
      shutil.copyfile("wrap_function.h.in",header_filename)
    ...
    func_decl+=indent+'ADD_METHOD('+parse_function.FunctionUsedName(funcname)+',"{0}  ({1})")\n'.format(\
          dh.GetDescription(),\
          WxHelpLink(funcname))
    if missingtypes!="":
      func_decl += "*/\n"
    ...
    if parse_function.IsTemplate(funcname):
      shutil.copyfile("wrap_functiontemplate.cpp.in",impl_filename)
    else:
      shutil.copyfile("wrap_function.cpp.in",impl_filename)
