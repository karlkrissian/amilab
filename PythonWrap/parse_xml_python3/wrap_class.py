...
if args.val.classes_includes != '':
    print("-------- args.classes_includes -------------")
    loader = importlib.machinery.SourceFileLoader('module_classes_includes', args.val.classes_includes)
    module_classes_includes = loader.load_module()
    classes_inc = module_classes_includes.classes_includes

if args.val.members_blacklist != '':
    # print "-------- args.classes_includes -------------"
    loader = importlib.machinery.SourceFileLoader('module_mem_blacklist', args.val.members_blacklist)
    mem_blacklist = loader.load_module()
    ...
    try:
        if args.val.classes_includes != '':
            ...
            if classname in classes_inc.classes_includes:
                ...
                for pattern in list(classes_inc.classes_includes.keys()):
...
def AvailableType(typename, typeid, missing_types, check_includes=False, return_type=False):
    ...
    try:
        typeid_type = config.types[typeid]
    except:
        missing_types.append(typename)
        return False
    if check_includes:
        if typename in config.available_classes and typeid in config.types:
            # if typename.find('SurfacePoly')!=-1:
            # print "AddDeclare..."
            config.AddDeclare(typename)
            if typename == typeid_type.GetDemangled():
                # print "AvailableType for {0}".format(typename)
                try:
                    fileid = typeid_type.fileid
    ...
    if not (typename in config.available_classes) and \
            ...
            not (typename in typesubst.type_substitute) and \
            typename not in typesubst.type_equivalence:
        missing_types.append(typename)
        if (typeid_type.GetType() in config.class_types) \
                and (typename not in config.needed_classes) \
                and (typename not in config.new_needed_classes):
            # add to class list
            config.new_needed_classes.append(typename)
...
def MissingTypes(classname, method, check_includes=False):
    ...
    for a in method.args:
        try:
            typeid_type = config.types[a.typeid]
            ...
            if typefullname == None:
                ...
                if typename == 'void' and ispointer:
                    ...
                    if (typefullname.endswith("* * *")) or (typefullname.endswith("* const *")):
                        ...
                        typeid = typeid_type.GetMainTypeId()
                        ...
                        if shared_type != None:
                            ...
                            if not avail:
                                ...
                                # in its full name
                                typedefname = typeid_type.GetString()
                        ...
                        if (not avail):
                            utils.WarningMessage(
                                "type {0} not available: {1}".format(typename, config.types[typeid].GetType()))
        except:
            missing_types.append(a.typeid)
...
class MethodInfo:
    ...
    def GetDescription(self, classname, constructor=False):
        ...
        if self.static == 1:
            res += "static "
        if self.returntype != None and self.returntype in config.types:
            res += config.types[self.returntype].GetFullString() + " "
        if self.name in config.available_operators:
        ...
        for a in self.args:
            ...
            if a.typeid in config.types:
                fullstring = config.types[a.typeid].GetFullString()
                if fullstring != None:
                    res += fullstring + " " + a.name
                    if a.default != None:
...
def CheckBlackList(mname, demangled):
    try:
        ...
        print(f"Exception catched during CheckBlackList {mname}: {e}")
        pass
...
class ParsePublicMembers:
    ...
    def CheckOperatorMethodName(self, method):
        ...
        if mname == '++' or mname == '--':
            ...
            logging.info(
                "CheckOperatorMethodName() {0}: method.usedname = {1}".format(method.demangled, method.usedname))
            ...
            if num == 1:
                ...
                for m in self.public_members.OperatorMethods:
                    if m.name == mname and mname in config.available_operators:
                        self.public_members.OperatorMethods[pos].usedname += "_1"
                    pos = pos + 1
                # add main method
                method = MethodInfo()
                method.name = mname
                if mname in config.available_operators:
                    method.usedname = config.available_operators[mname]
                else:
    ...
    def startElement(self, name, attrs):
        # print "ParsePublicMembers.startElement()"
        if self.inmethod == 1:
        ...
        if name == 'Field':
            ...
            discard = False
            if "{0}::{1}".format(context_type.GetDemangled(), fname) in config.members_blacklist:
                discard = True
            try:
                if "{0}::{1}".format(context_type.GetDemangled(), fname) in \
        ...
        if name == 'Typedef':
            ...
            typedef.demangled = context_type.GetDemangled() + "::" + tname
        ...
        if name == 'Constructor':
            ...
            if mname == None or config.types[context].GetString().find(mname) == -1:
                utils.WarningMessage(
                    " replacing constructor name {0} --> {1}".format(mname, config.types[context].GetString()))
                mname = context_type.GetString()
...
def AddParameters(method, numparam=-1):
    res = '('
    if (len(method.args) > 0):
        if numparam != 0 and method.args[0].typeid in config.types:
            res += '({0}) {1}'.format(
        ...
        for i in range(1, len(method.args)):
            if numparam == -1 or i < numparam:
                ...
                if method.args[i].typeid in config.types:
                    res += ', ({0}) {1}'.format(
...
def ImplementMethodDescription(classname, method, constructor=False, methodcount=1):
    ...
    for a in method.args:
        ...
        if a.typeid in config.types:
            ...
            if typename in typesubst.type_substitute:
            ...
            if typename.count(",") > 0:
                res += '  ADDPARAMCOMMENT_TYPE( {0}, "parameter named \'{1}\''.format(
                    config.ClassTypeDef(typename_comment), a.name)
            else:
            ...
            print("unknown type")
    if method.returntype != None and method.returntype in config.types:
        maintypeid = config.types[method.returntype].GetMainTypeId()
        returntypest = config.types[maintypeid].GetString()
...
def ImplementMethodCall(classname, method, numparam, constructor=False, ident=''):
    ...
    if constructor:
        ...
        if method.returntype not in config.types:
            return "unknown type"
        ...
        if (returntypest == "void") and (not returnpointer):
            ...
            # --- No type substitution before return
                ...
                if typename in config.available_classes and returnpointer:
                    nonconstres = typesubst.RemovePointerConstness(config.types[method.returntype].GetFullString(),
                                                                   "res")
                    # don't delete returned pointer ...
                    res += ident + '  if ({0}==NULL) return nullvar;\n'.format(nonconstres)
                    res += ident + '  BasicVariable::ptr res_var = AMILabType<{0} >::CreateVar({1},true);\n'.format(
                        typename, nonconstres)
                    res += ident + '  return res_var;\n'
                else:
                    if returnpointer:
                        res += ident + '  if ({0}==NULL) return nullvar;\n'.format('res')
            ...
            for n in range(1, nummethods + 1):
                ...
                for m in methods:
                    ...
                    if m.usedname == usedname and m.const == constness:
                        ...
                        utils.WarningMessage(
                            " Duplicated Method {0} is implemented: {1}".format(usedname, is_implemented))
                    pos = pos + 1
                if is_implemented:
                    if m.static == "1":
...
#  WrapClass
# ----------------------------------------------------------------------
def WrapClass(classname, include_file, xmltree):
    if (args.val.profile):
        t0 = time.time()
        # print "\n**************"
    ...
    found = classname in config.classes
    if not found:
        # Check for possible typedef of class or struct
        if classname in config.typedefs:
            typeid = config.typedefs[classname]
            typeinfo = config.types[config.typedefs[classname]]
            refid = typeinfo.GetRefTypeId()
            reftype = config.types[refid]
            if reftype.EmptyName() and reftype.GetType()=='Struct':
                # Add the typedef to the list of classes for the wrapping
                config.classes[classname] = config.classes[reftype.GetName()]
                refclassname = reftype.GetName()
                found = True
    else:
    ...
    if found:
        ...
        if refclassname not in config.parsed_classes:
            logging.debug("*** Find Public Members")
            fpm = FindPublicMembers([refclassname])
            fpm.parse(xmltree)
        ...
        if dh.has_copyconstr:
            ...
            if (IsTemplate(classname) and args.val.templates) or IsWithinContext(classname):
                implement_type += "AMI_DEFINE_VARFROMSMTPTR_TEMPLATE2({0},{1});\n".format(
                    config.ClassTypeDef(classname), config.ClassUsedName(classname))
            else:
        ...
        for m in fm.Methods:
            ...
            if m.usedname in list(config.ami_tokens.values()):
        ...
        if m != None:
            ...
            for template_param in template_params:
                ...
                if template_param in config.classes:
            ...
            if wrapped_constructors > 0:
                ...
                if args.val.constructor != '' and \
                    ...
                    implement_createvar += "  WrapClass_{0}::wrap_static_{1} construct;\n".format(
                        config.ClassUsedName(classname), args.val.constructor)
                    implement_createvar += "  construct.Set_quiet(quiet);\n"
                    implement_createvar += "  return construct.CallMember(p);\n"
                else:
    ...
    if found:
        ...
        print("Classes are:")
        print("****************************")
        print(list(config.classes.keys()))
        print("****************************")
    ...
    if (args.val.profile):
        if not found:
            ...
            print("WrapClass({0})  {1}".format(classname, time.time() - t0))
        t0 = time.time()
