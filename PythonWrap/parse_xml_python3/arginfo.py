...
class ArgInfo:
    ...
    def WrapGetParamValue_subst(self):
        ...
        res += "  " + \
            typesubst.ConvertValTo(self.typeid, substvar, self.name)+"\n"
        # {1} {0} = ({1}) ({2}>0.5);\n".format(self.name,typename,substvar)
        return res

    def WrapGetParamValue(self, noconstructor_call):
        if self.typename in list(typesubst.type_substitute.keys()):
    ...
    def WrapGetParamPointer_subst(self):
        ...
        conv_res = typesubst.ConvertSmtPtrToPtr(
            self.typeid, substvar, self.name)
        if conv_res != "":
            res += "  " + \
                typesubst.ConvertSmtPtrToPtr(
                    self.typeid, substvar, self.name)+"\n"
            # res += "  {1}* {0} = &{0}_val;\n".format(self.name,self.typename)
        else:
            print("Smartpointer to pointer conversion failed for type {0}!".format(
                self.typename))
            # otherwise convert from value, but could only work for constants???
            res += "  "+typesubst.ConvertValTo(self.typeid, "(*{0})".format(
                substvar), "{0}_val".format(self.name))+"\n"
            res += "  {1}* {0} = &{0}_val;\n".format(self.name, self.typename)
        return res
    ...
    def WrapGetParamDoublePointer_subst(self):
        ...
        res += "  if (!AMILabType<{0} >::get_val_smtptr_param({1},_p,_n,true,false,{3})) {2};\n".format(
            substype, substvar, self.returnstring, self.quiet)
        # Try to create pointer from smart pointer if available
        conv_res = typesubst.ConvertSmtPtrToDoublePtr(
            self.typeid, substvar, self.name)
        if conv_res != "":
            res += "  "+conv_res+"\n"
            # res += "  {1}* {0} = &{0}_val;\n".format(self.name,self.typename)
    ...
    def WrapGetParamRef(self, noconstructor_call):
        ...
        res = "  boost::shared_ptr<{0} > {1}_smtptr;\n".format(
            smtptr_type, self.name)
        ispointer = fulltype.startswith(
            self.typename+" *") or fulltype.startswith(self.typename+" const *")
        if noconstructor_call:
            noconst = ',true'
        else:
    ...
    def WrapGetParamPost_Pointer_bool(self):
        res = "  "+typesubst.ConvertValFrom_bool("(*{0})".format(
            self.name), "*{0}_{1}".format(self.name, "int"))+"\n"
        return res
    ...
    def WrapGetParamPost_Pointer(self):
        ...
        if typename in list(typesubst.type_substitute.keys()):
            if typename == "bool":
