...
def GetShortName(typename):
    if typename in list(type_shortname.keys()):
...
def ConvertValFrom_uint(typevar, substvar):
    ...
    # should check limits
    res = "{1} {0} = boost::numeric_cast<{1} >((unsigned int){2});".format(
        substvar, substtype, typevar)
    return res
...
def ConvertValTo_uint(substvar, typevar):
    ...
    config.AddInclude('#include "boost/numeric/conversion/cast.hpp"\n')
    # should check limits
    res = "{1} {0} = boost::numeric_cast<{1} >({2});".format(
        typevar, typename, substvar)
    return res
...
def ConvertSmtPtrToPtr_char(typeid, substvar, typevar):
    ...
    if fulltypename == "char *":
        ...
        if fulltypename == "char * const":
            ...
            res = "{0} {1} = ({0}) {2}->c_str();".format(fulltypename,
                                                         typevar, substvar)
        else:
            res = "{0} {1} = {2}->c_str();".format(fulltypename,
                                                   typevar, substvar)
...
def ConvertSmtPtrToDoublePtr_char(typeid, substvar, typevar):
    ...
    res += "{0} {1} = ({0}) &{1}_pointer;".format(fulltypename,
                                                  typevar, substvar)
    # print "res = {0}".format(res)
    return res
...
def ConvertValFrom_wchar_t(typevar, substvar):
    ...
    res += indent + \
        "  if (conv_res>0) {0} = std::string(1,char_conv);\n".format(substvar)
    res += indent+"}"
    return res
...
def ConvertPtrFrom_wchar_t(typevar, substvar):
    ...
    res += indent + \
        "  if (conv_res>0) {0} = std::string(char_conv);\n".format(substvar)
...
def ConvertPtrFrom(typeid, typevar, substvar):
    ...
    try:
        ...
        print(
            "Warning: pointer conversion failed for type {0}".format(typename))
