...
def WrapMethodTypePointer(typedefname, include_file):
    ...
    shutil.copyfile(args.val.templatefile_dir +
                    "/wrap_class.h.in", header_filename)
    ...
    shutil.copyfile(args.val.templatefile_dir +
                    "wrap_class.cpp.in", impl_filename)
    ...
    for line in fileinput.FileInput(impl_filename, inplace=1):
        ...
        line = line.replace("${IMPLEMENT_SMART_POINTER}",
...
def process_ancestors():
    ...
    for f, f_type in config.types.items():
    ...
    for a in ancestors:
        ...
        if a in list(fpm.parse_public_members.demangled_typedefs.keys()):
            ...
            if refid in config.types:
                print("1.adding '{0}' ".format(
                    config.types[refid].GetFullString()))
        ...
        if wrap_class.IsTemplate(b):
            ...
            for nt in ttypes:
                ...
                if nt not in ancestors_templates:
                    if nt in list(classes_dict.values()) and \
    ...
    for b in ancestors:
        ...
        for f in list(classes_dict.keys()):
            if classes_dict[f] == b:
                if b.find("stream") != -1:
                ...
                while f_anc != []:
                    anc_id = f_anc.pop()[0]
                    if b.find("stream") != -1:
                        print("processing base {0}".format(
                            config.types[anc_id].GetFullString()))
                    # if b=="wxTopLevelWindow":
                    # print anc_id
                        ...
                        if classes_dict[anc_id] not in ancestors and \
                            ...
                            if m != None and not exclude:
                                ...
                                if main_anc_id != anc_id:
                                    if b.find("stream") != -1:
                                        print("anc_id {0}, main_anc_id {1}".format(
                                            anc_id, main_anc_id))
    ...
    for a in ancestors:
        ...
        if len(a) > 100 or a == "_0":
            print(
                "Rejecting {0} from ancestors (too long name or '_0')\n".format(a))
            continue
        ...
        if a in list(classes_dict.values()):
            # print "*"
            pos = list(classes_dict.values()).index(a)
            # print " pos = {0} id = {1}".format(pos,classes_dict.keys()[pos])
            fid = config.types[list(classes_dict.keys())[pos]].fileid
            # print "fid=",fid
...
if __name__ == '__main__':
    ...
    if not (os.path.exists(args.val.outputdir)):
        try:
            ...
            print("{0} failed: switch to temporary output directory {1}".format(
                prev, args.val.outputdir))
    ...
    for class_id in config.types:
        ...
        if class_arg.GetType() == 'Typedef':
            # if typedef to an unamed struct
            demangled_name = class_arg.GetDemangled()
    ...
    saveconf = open(args.val.outputdir+"/saveconfig.dat", "wb")
    pickle.dump(config.types, saveconf)
    ...
    utils.WarningMessage(
        "Number of files found : {0}".format(ft.number_of_files))
    ...
    if n >= nmax:
        print(
            "WARNING: Exceeding the maximum number of classes to wrap which is set to {0} !!\n".format(nmax))
    ...
    if args.val.libname != None and args.val.addwrap:
        # header
        createcontextname = args.val.outputdir + \
            "/addwrap_{0}.h.new".format(args.val.libname)
        f = open(createcontextname, "w")
        # 1. write some doc
        f.write("/*\n")
        f.write(
            " * Automatic wrapping of '{0}' library for AMILab.\n".format(args.val.libname))
        ...
        # implementation
        createcontextname = args.val.outputdir + \
            "/addwrap_{0}.cpp.new".format(args.val.libname)
        f = open(createcontextname, "w")
        # 1. write some doc
        f.write("/*\n")
        f.write(
        ...
        f.write('\n')
        f.write(
            "// Currently {0} functions are wrapped \n".format(len(lib_functions)))
        for func in lib_functions:
            f.write('#include "wrap_{0}.h"\n'.format(
                config.ClassUsedName(func)))
        ...
        for t in list(config.types.keys()):
            if config.types[t].GetType() == "Enumeration":
                enumkeys = list(config.types[t]._values.keys())
                ...
                if wrap_enum:
                    ...
                    enum_usedname = config.types[t].GetName().replace(
                        '.', 'enum')
                    if enum_usedname == "":
