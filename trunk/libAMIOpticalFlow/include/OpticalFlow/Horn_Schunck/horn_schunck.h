#ifndef _HORN_SCHUNCK_H
#define _HORN_SCHUNCK_H

//Esta clase implementa el m�odo de Horn & Schunck multipiramidal
...
//          print << I2s << nx << ny;
  ...
  private:
     ...
     int compute_implicit(const image &I1, const image &I2, image &u, image &v,
        ...
        const int size = nx * ny;
        const real C   = computeC2(I2, nx, ny, alpha); //computeC(I2, nx, ny, alpha);
        ...
        for(real t = 0; t < T; t += dt) {
           ...
           NM_AMI_UTIL::warp<real> W;
           W(I2,  u, v, I2w,  nx, ny);
           W(I2x, u, v, I2xw, nx, ny);
           W(I2y, u, v, I2yw, nx, ny);
           ...
           while(error > TOL) {
              ...
              for(int j = 0; j < ny; j++)
                ...
                for(int k=0; k < nx; k++){
                 ...
                 const real Du   = (4 + C * I2xw[i] * I2xw[i]);
                 const real Nu   = C * (diff + uk[i] * I2xw[i] - I2yw[i] * difv) * I2xw[i] +
                                   div(u, k, j, nx, ny);

                 u[i] = (u[i] + dt * Nu ) / ( 1. + dt * Du );

                 const real difu = (u[i] - uk[i]);
                 const real Dv   = (4 + C * I2yw[i] * I2yw[i]);
                 const real Nv   = C * (diff - I2xw[i] * difu + vk[i] * I2yw[i]) * I2yw[i] +
                                   div(v, k, j, nx, ny);
     ...
     int compute_explicit(const image &I1, const image &I2, image &u, image &v,
        ...
        for(real t = 0; t < T; t += dt) {
           ...
           for(int i = 0; i < size; i++) {
                 ...
                 const real diff = I1[i] - I2w[i];
                 const real Nu = C * diff * I2xw[i] + div_u[i];
                 const real Nv = C * diff * I2yw[i] + div_v[i];
                 u[i] = uk[i] + dt * Nu ;
                 v[i] = vk[i] + dt * Nv ;
           }
     ...
     real computeC(const image &I, const int nx, const int ny, const real alpha) {

       const int size = nx * ny;

       real *Ix = new real[size];
       real *Iy = new real[size];

       NM_AMI_MATH::gradient<real> gradiente;
       gradiente(I, Ix, Iy, nx, ny);
     ...
     template<class image>
     real computeC2(const image &I, const int nx, const int ny, const real alpha) {
