...
class zoom{
       ...
       real value (const image &I, const int nx, const int ny, const int i,
                   const int j, const real factor) {
    ...
    public:
        ...
        template <typename image>
        int operator () (const image &I, image &Iout, const int nx, const int ny,
                         const real factor){
        ...
        int operator () (const image &I, image &Iout, const int nx, const int ny,
            ...
            const real factor1 = ((real)nxx / nx);
            const real factor2 = ((real)nyy / ny);
            const real factor  = std::max(factor1, factor2);
