...
template<class T>
void InrImageIterator<T>::BufferPos( int x, int y, int z) 
...
template<class T>
bool InrImageIterator<T>::operator++()
{
...
template<class T>
bool InrImageIterator<T>::Inc( const int& inc)
{
  _pos_buf +=inc;
  if  (_pos_buf >= _imsize) return false;
  _datapos += _vdim*inc;
...
template<class T>
void InrImageIterator<T>::ValueInc( const int& inc);
{
  _datapos += inc;
}

template<class T>
double InrImageIterator<T>::GetDoubleValue( int field )
{
  return (double) _datapos[field];
}

template<class T>
double InrImageIterator<T>::GetIncDoubleValue( const int& inc, const int& field ) const
{
  return (double) _datapos[_vdim*inc+field];
}

template<class T>
void InrImageIterator<T>::SetDoubleValue( const double& val)
{
  *_datapos = (T) val;
  if (_vdim>1) {
...
template<class T>
T InrImageIterator<T>::GetValue( int field ) const
{
  return _datapos[field];
}

template<class T>
void InrImageIterator<T>::SetValue( const T& val)
{
  *_datapos = val;
}
...
template<class T>
void InrImageIterator<T>::SetDoubleValue( int field, const double& val)
{
...
template<class T>
void InrImageIterator<T>::SetValue( int field, const T& val)
{
  _datapos[field] = val;
}
