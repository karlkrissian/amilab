...
BEGIN_EVENT_TABLE (StcTestFrame, wxFrame)
    // common
    EVT_CLOSE (                      StcTestFrame::OnClose)
    // file
    EVT_MENU (wxID_OPEN,             StcTestFrame::OnFileOpen)
    EVT_MENU (wxID_SAVE,             StcTestFrame::OnFileSave)
    EVT_MENU (wxID_SAVEAS,           StcTestFrame::OnFileSaveAs)
    EVT_MENU (wxID_CLOSE,            StcTestFrame::OnFileClose)
    // properties
    EVT_MENU (myID_PROPERTIES,       StcTestFrame::OnProperties)
    // print and exit
    EVT_MENU (wxID_PRINT_SETUP,      StcTestFrame::OnPrintSetup)
    EVT_MENU (wxID_PREVIEW,          StcTestFrame::OnPrintPreview)
    EVT_MENU (wxID_PRINT,            StcTestFrame::OnPrint)
    EVT_MENU (wxID_EXIT,             StcTestFrame::OnExit)
    // edit
    EVT_MENU (wxID_CLEAR,            StcTestFrame::OnEdit)
    EVT_MENU (wxID_CUT,              StcTestFrame::OnEdit)
    EVT_MENU (wxID_COPY,             StcTestFrame::OnEdit)
    EVT_MENU (wxID_PASTE,            StcTestFrame::OnEdit)
    EVT_MENU (myID_INDENTINC,        StcTestFrame::OnEdit)
    EVT_MENU (myID_INDENTRED,        StcTestFrame::OnEdit)
    EVT_MENU (wxID_SELECTALL,        StcTestFrame::OnEdit)
    EVT_MENU (myID_SELECTLINE,       StcTestFrame::OnEdit)
    EVT_MENU (wxID_REDO,             StcTestFrame::OnEdit)
    EVT_MENU (wxID_UNDO,             StcTestFrame::OnEdit)
    // find
    EVT_MENU (wxID_FIND,             StcTestFrame::OnEdit)
    EVT_MENU (myID_FINDNEXT,         StcTestFrame::OnEdit)
    EVT_MENU (myID_REPLACE,          StcTestFrame::OnEdit)
    EVT_MENU (myID_REPLACENEXT,      StcTestFrame::OnEdit)
    EVT_MENU (myID_BRACEMATCH,       StcTestFrame::OnEdit)
    EVT_MENU (myID_GOTO,             StcTestFrame::OnEdit)
    // view
    EVT_MENU_RANGE (myID_HILIGHTFIRST, myID_HILIGHTLAST,
                                     StcTestFrame::OnEdit)
    EVT_MENU (myID_DISPLAYEOL,       StcTestFrame::OnEdit)
    EVT_MENU (myID_INDENTGUIDE,      StcTestFrame::OnEdit)
    EVT_MENU (myID_LINENUMBER,       StcTestFrame::OnEdit)
    EVT_MENU (myID_LONGLINEON,       StcTestFrame::OnEdit)
    EVT_MENU (myID_WHITESPACE,       StcTestFrame::OnEdit)
    EVT_MENU (myID_FOLDTOGGLE,       StcTestFrame::OnEdit)
    EVT_MENU (myID_OVERTYPE,         StcTestFrame::OnEdit)
    EVT_MENU (myID_READONLY,         StcTestFrame::OnEdit)
    EVT_MENU (myID_WRAPMODEON,       StcTestFrame::OnEdit)
    // extra
    EVT_MENU (myID_CHANGELOWER,      StcTestFrame::OnEdit)
    EVT_MENU (myID_CHANGEUPPER,      StcTestFrame::OnEdit)
    EVT_MENU (myID_CONVERTCR,        StcTestFrame::OnEdit)
    EVT_MENU (myID_CONVERTCRLF,      StcTestFrame::OnEdit)
    EVT_MENU (myID_CONVERTLF,        StcTestFrame::OnEdit)
    EVT_MENU (myID_CHARSETANSI,      StcTestFrame::OnEdit)
    EVT_MENU (myID_CHARSETMAC,       StcTestFrame::OnEdit)
    // help
    EVT_MENU (wxID_ABOUT,            StcTestFrame::OnAbout)
    //Event when close a tab
    EVT_AUINOTEBOOK_PAGE_CLOSE(wxID_NOTEBOOK, StcTestFrame::OnCloseTab)
END_EVENT_TABLE ()

//----------------------------------------------------------------------------
//epf contructor & destructor
epf::epf (Edit* e, wxWindow* p, wxString f) {
  ed = e;
  pag = p;
  filena = f;
}
...
void tabsVector::addTab (Edit* editor, wxWindow* page, const wxString& filename) {
...
Edit* tabsVector::getEdit (wxWindow* page) {
  vector<epf>::iterator it;
  for (it = vec.begin(); it != vec.end() ; it++) {
    if (page == it->getPag()) {
      return it->getEd();
...
wxWindow* tabsVector::getPage (Edit* ed) {
  vector<epf>::iterator it;
  for (it = vec.begin(); it != vec.end(); it++) {
    if (ed == it->getEd()) {
      return it->getPag();
...
/*void StcTestFrame::OnClose (wxCloseEvent &event) {
    wxCommandEvent evt;
    OnFileClose (evt);
...
void StcTestFrame::OnAbout (wxCommandEvent &WXUNUSED(event)) {
    AppAbout dlg(this);
}

void StcTestFrame::OnExit (wxCommandEvent &WXUNUSED(event)) {
...
// file event handlers
/*void StcTestFrame::OnFileOpen (wxCommandEvent &WXUNUSED(event)) {
    if (!m_edit) return;
#if wxUSE_FILEDLG
    wxString fname;
...
/*void StcTestFrame::OnFileSave (wxCommandEvent &WXUNUSED(event)) {
    if (!m_edit) return;
    if (!m_edit->Modified()) {
...
/*void StcTestFrame::OnFileSaveAs (wxCommandEvent &WXUNUSED(event)) {
    if (!m_edit) return;
#if wxUSE_FILEDLG
...
/*void StcTestFrame::OnFileClose (wxCommandEvent &WXUNUSED(event)) {
    if (!m_edit) return;
    if (m_edit->Modified()) {
...
/*void StcTestFrame::OnProperties (wxCommandEvent &WXUNUSED(event)) {
    if (!m_edit) return;
...
void StcTestFrame::OnPrintSetup (wxCommandEvent &WXUNUSED(event)) {
#if wxUSE_PRINTING_ARCHITECTURE
...
/*void StcTestFrame::OnPrintPreview (wxCommandEvent &WXUNUSED(event)) {
#if wxUSE_PRINTING_ARCHITECTURE
...
/*void StcTestFrame::OnPrint (wxCommandEvent &WXUNUSED(event)) {
#if wxUSE_PRINTING_ARCHITECTURE
    wxPrintDialogData printDialogData( *g_printData);
...
// edit events
/*void StcTestFrame::OnEdit (wxCommandEvent &event) {
...
Edit* StcTestFrame::getActiveEditor () {
...
void StcTestFrame::OnCloseTab (wxAuiNotebookEvent & event) {
  if (!notebook) return;
  //If there aren't tabs, close editor directly
  if (notebook->GetPageCount() == 0) {
    Destroy();
...
/*void StcTestFrame::FileOpen (wxString fname)
{
...
wxRect StcTestFrame::DeterminePrintSize () {
