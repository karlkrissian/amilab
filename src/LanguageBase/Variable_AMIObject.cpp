...
BasicVariable::ptr Variable<AMIObject>::operator =(const BasicVariable::ptr& b)
{
  ...
  if (b->Type()==type_image) {
    ...
    if (var_im2.get()) {
      // copy option
      if (!((*Pointer())=(*var_im2->Pointer()))) GB_DriverBase->err_print(" Error in images assignement\n");
    } else {
      if (GB_DriverBase->err_print("Error, empty variable as parameter of assignment operator\n"));
    }
  } 
  else
    CLASS_ERROR("operation not defined");
