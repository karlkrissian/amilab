    ...
    wrap_wxColour::CallMember( ParamList* p)
{
  ...
  if (p->GetNumParam()==0) 
    return WrapClass_wxColour::CreateVar(new wxColour());
...
{
  return WrapClass_wxColour::CreateVar( new wxColour(*(_objectptr->_obj)));
}
...
{
  int n = 0;
  CLASS_GET_OBJECT_PARAM2(wxColour,var,_obj);
