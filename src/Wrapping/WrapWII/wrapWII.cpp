...
void WII_GetRoll(ParamList* p) {
  ...
  if (!get_floatvar_param(    rollp,         p, n)) HelpAndReturn;
...
void WII_GetPitch(ParamList* p) {
  ...
  if (!get_floatvar_param(    pitchp,         p, n)) HelpAndReturn;
...
void WII_GetGForceX(ParamList* p) {
  ...
  float* gforce;
  int n=0;
  if (!get_floatvar_param(    gforce,         p, n)) HelpAndReturn;
  *gforce = wii_gforce.x;
}
...
void WII_GetButtons(ParamList* p) {
  ...
  int* bts;
  int n=0;
  if (!get_intvar_param(    bts,         p, n)) HelpAndReturn;
  *bts = wii_btns;
}
...
void WII_GetEventCounter(ParamList* p) 
{
  ...
  float* counter;
  int n=0;
  if (!get_floatvar_param(    counter,         p, n)) HelpAndReturn;
...
void WII_SetMotionSensing(ParamList* p)
{
  ...
  float ms;
  int n=0;
  if (!get_float_param(  ms,         p, n)) HelpAndReturn;
