...
#include <Windows.h.>
#include <direct.h>
#include <errno.h> // esta  línea es una modificación para que compile en visual 2008
...
GuiGenerator::GuiGenerator()
{
	...
	this->isXPointsProcessingEnabled = false;
...
void GuiGenerator::AdjustLigtCoolnessByCoolCard()
{
	...
	for (markerNum = 1; markerNum <= markersCollection->count(); markerNum++)
	{
		...
		if (m->wasIdentified(this->pCurrCam) )
		{
		   if ( strncmp( m->getName(),"COOL",4 ) == 0 ||  
		   	...
		   	( strncmp( m->getName(),"Cool",4 ) == 0)) {
		      MTCollection::Collection* facetsCollection = 
		   	   new MTCollection::Collection(m->identifiedFacets(this->pCurrCam));
		      Facet* f = new Facet(facetsCollection->itemI(1));
		      ColorVector = (f->IdentifiedVectors())[0];
		   }
		}
		free(m);
	}
	if ( ColorVector != NULL) {
		this->pPers->setSection ("General");
		this->pCurrCam->AdjustCoolnessFromColorVector(ColorVector->Handle());
...
void GuiGenerator::startGrabingFrames()
{
	...
	for ( i=0 ;i< this->pCameras->getCount();i++) {
		...
		cbuffer =  (int *)malloc(2*sizeof(int));
		cbuffer[0] = (int)this;
		cbuffer[1] = (int) this->pCameras->m_vCameras[i];	
