 ...
 */
double ModeleReconstruction(  double distance, 
//         --------------------
                                  double rayon)
{
    ...
    if (ps < 0) {
      ...
      if (ps < 0) {
        ...
        if (distance < facteur_cube*rayon1) {
          recons->BufferPos( x, y, z);
          valeur = ModeleReconstruction( distance, rayon1);
          recons->FixeValeur( sup(valeur,
                                  recons->ValeurBuffer()));
        }
      }
                      ...
                      InrImage* radii)
{
  ...
  recons = new InrImage( WT_FLOAT,1,"recons.ami.gz", ref);
