...
SRAD::NoiseEstimationMode SRAD::noise_estimation_mode = SRAD::MODE_AUTO;

//------------------------------------------------------------------------------
double SRAD::Cmad(InrImage* im)
//
{
...
//------------------------------------------------------------------------------
double SRAD::Compute_q0_subvol(InrImage* im, int mode, ami::ImageExtent<float>* extent)
//
{
  InrImage* subvol;
  //  InrImage* im1;
  ...
  switch (mode) {
    case SRAD::MODE_LEE:
    case SRAD::MODE_KUAN:  if (mean>0) return var/mean/mean;   else return 0;
    case SRAD::MODE_ADDITIVE: return var;
  }
  return 0;
}
...
//------------------------------------------------------------------------------
double SRAD::Compute_q0_2(InrImage* im, int mode, ami::ImageExtent<float>* extent)
//
{
...
//--------------------------------------------------------------------------------
double SRAD::Compute_sigma2_MRI(InrImage* im, ami::ImageExtent<float>* extent)
//
{
  InrImage* subvol;
  //  InrImage* im1;
...
//--------------------------------------------------------------------------------
double SRAD::Compute_sigma2_MRI_mode( InrImage* im, 
...
//------------------------------------------------------------------------------
double SRAD::Compute_q0_2(InrImage* im, 
                          InrImage* meanI, 
                          InrImage* meanI2, 
                          int mode, 
                          int neighborhood)
...
//------------------------------------------------------------------------------
inline double SRAD::function_c(double q_2, double q0_2)
//     ----------
{
...
//--------------------------------------------------------------------------------
inline double SRAD::function_c_Kuan(double q_2, double q0_2)
//     ---------------
{
...
//--------------------------------------------------------------------------------
inline double SRAD::function_c_MRI(double sigma2, double vg, double meang)
//     ---------------
{
...
//--------------------------------------------------------------------------------
inline double SRAD::function_c_additive(double q_2, double q0_2)
//     -------------------
{
...
//------------------------------------------------------------------------------
inline double SRAD::function_q2(double dI_I_2, double d2I_I) {
//     ----------
  double tmp;
...
//------------------------------------------------------------------------------
inline double SRAD::function_q0(double t, double rho, double q0) {
  return q0*exp(-rho*t);
}

//------------------------------------------------------------------------------
void SRAD::Compute_q_coeff(InrImage* image_in, InrImage* image_q ){
//   ---------------
...
//------------------------------------------------------------------------------
void SRAD::Compute_d_coeff( InrImage* image_res,InrImage* image_c,InrImage* image_d, 
                      double& dt1, int mode, 
                      int neighborhood, ami::ImageExtent<float>* extent) 
{
  ...
  for (x=0;x<tx;x++) {
    ...
    switch (mode) {
      case SRAD::MODE_LEE:      
        image_c->FixeValeur( function_c(         q2, q0_2)); break;
      case SRAD::MODE_KUAN:     
        image_c->FixeValeur( function_c_Kuan(    q2, q0_2)); break;
      case SRAD::MODE_ADDITIVE: 
        image_c->FixeValeur( function_c_additive(mean2-mean1*mean1,q0_2)); 
        break;
    }
...
//------------------------------------------------------------------------------
InrImage::ptr SRAD::qcoeff( InrImage* input) 
{

  
    InrImage* image_in;
    InrImage::ptr image_q;
    //    char resname[100];
    //    double t;
    //    int n;
  ...
  image_q = InrImage::ptr(new InrImage( WT_FLOAT, "image_q.ami.gz", input));
  Compute_q_coeff(image_in,image_q.get());
...
//------------------------------------------------------------------------------
// explicit scheme
InrImage::ptr SRAD::SRAD_ver1(  InrImage* input, float dt, int numit, 
                                EquationType mode,  
                                int neighborhood,
                                ami::ImageExtent<float>* extent) 
{
...
//------------------------------------------------------------------------------
void SRAD::UpdateResult2( InrImage* image_res,
                          float dt, 
                          InrImage* image_c, 
                          InrImage* image_d, 
                          int neighborhood, 
                          ...
                          ami::ImageExtent<float>* extent) 
{
  ...
  switch (mode) {
    ...
    case SRAD::MODE_KUAN:
      ...
      break;
    case SRAD::MODE_ADDITIVE:
      if (noise_estimation_mode==SRAD::MODE_AUTO) {
        q0_2= Compute_q0_2(image_res,mode, extent);
        printf("q0_2 = %f \n",q0_2);
      } else
      if (noise_estimation_mode==SRAD::MODE_HISTOMODE) {
        q0_2 =  Compute_sigma2_MRI_mode(image_res,extent,false);
        printf("q0_2_test = %f \n",q0_2);
      } else
        std::cerr << "Unknown noise estimation mode" << std::endl;
      break;
    case SRAD::MODE_MRI:
    case SRAD::MODE_MRI_NEW:
...
//
InrImage::ptr SRAD::SRAD_ver2( InrImage* input, float dt, int numit, 
                       EquationType mode, 
                       int neighborhood,
                       ami::ImageExtent<float>* extent)
{
...
InrImage::ptr SRAD::AnisoDPAD(  InrImage* input, 
                                float dt, 
                                int numit, 
                                int neighborhood,
                                ami::ImageExtent<float>* extent)
{
  ...
  res = SRAD::SRAD_ver1(  input,
                          dt,
                          numit,
                          (ami::SRAD::EquationType)1, // Kuan's function
                          neighborhood, 
                          extent);
  return res;
}
...
//------------------------------------------------------------------------------
// Compatibility function
InrImage::ptr SRAD::AnisoDPAD2( InrImage* input, 
                                float dt, 
                                int numit, 
                                int neighborhood,
                                ami::ImageExtent<float>* extent)
...
//------------------------------------------------------------------------------
// Compatibility function
// notice the change of the order of the parameters
InrImage::ptr SRAD::AnisoNRAD(  InrImage* input, 
                                EquationType mode, 
                                float dt, 
                                int numit, 
                                int neighborhood,
...
//------------------------------------------------------------------------------
// Compatibility function
InrImage::ptr SRAD::AnisoLeeAdd2( InrImage* input, float dt, int numit,
                                  ami::ImageExtent<float>* extent)
{
  InrImage::ptr res;
  ...
  res = SRAD::SRAD_ver2(  input,
                          dt,
                          numit,
                          (ami::SRAD::EquationType)2, // Additive mode
                          0, // Yu neighborhood
                          extent);
  return res;
}
