...
//                   ---------
{
  ...
  if (inputImage == NULL) {
    ...
    if ((type != VTK_FLOAT)&&(input_output_array==NULL)) {
      ...
      vtkInformation* _info = inputImage->GetInformation();
      vtkImageData::SetScalarType( VTK_FLOAT, _info);
      vtkImageData::SetNumberOfScalarComponents(1, _info);
...
void vtkImagePropagateDist::PropagateDanielsson2D( )
{
    ...
    int                dxpn,dypn;
    int                i,j;
    register float known_dist_pos=0;
    register float known_dist_neg=0;
