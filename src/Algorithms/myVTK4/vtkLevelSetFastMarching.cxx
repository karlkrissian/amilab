...
void vtkLevelSetFastMarching::InitParam(vtkImageData* input, vtkImageData* output)
{
  ...
  vtkImageData::SetNumberOfScalarComponents(1,  T->GetInformation());
...
//----------------------------------------------------------------------
//
inline float vtkLevelSetFastMarching::ComputeValueDijkstra( 
//                                    --------------------
                                                            FM_TrialPoint& trial)
{
...
#define macro_swap(a,b) tmp=a; a=b; b=tmp;
  ...
  double            val;
  short             x = trial.x,y = trial.y,z = trial.z;
  int               pos = trial.impos;
...
{\
  ...
  if (x>0   ) CheckMin(  -1, invF)
  if (x<tx-1) CheckMin(   1, invF)
  if (y>0   ) CheckMin( -tx, invF)
  if (y<ty-1) CheckMin(  tx, invF)
  if (z>0   ) CheckMin(-txy, invF)
  if (z<tz-1) CheckMin( txy, invF)
