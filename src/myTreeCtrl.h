...
{
public:
    MyTreeItemData(boost::shared_ptr<Variable> lvar) : _var(lvar) { }

    boost::weak_ptr<Variable> GetVar() const { return _var; }

private:
    /// weak pointer to the Variable
    boost::weak_ptr<Variable> _var;
};
...
private:
    boost::weak_ptr<Variable> _currentmenu_var;
    DECLARE_EVENT_TABLE();
};
