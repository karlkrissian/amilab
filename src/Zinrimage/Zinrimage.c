...
#define MALLOC(mess,pt,type,size)  \
  pt = (type) malloc(size); 
// fprintf(stderr,"%s \t Allocation of %03.2f Mb \n",mess,1.0*(size)/1000000);
...
static char ***create3DArray(int ncols, int nrows, int nplanes, int bytes) {
  ...
  MALLOC( "Zinrimage:create3DArray",p3, char***, nplanes * nrows * ncols * bytes + j);
...
void ***index3DArray(const void *data, unsigned int ncols, unsigned int nrows,
  ...
  MALLOC("Zinrimage:index3DArray",array, void ***, nplanes * sizeof(void **) + nrows * nplanes * sizeof(void *));
...
void ****index4DArray(const void *data, unsigned int ncols, unsigned int nrows,
  ...
  MALLOC( "Zinrimage:index4DArray",array, void ****, vdim * sizeof(void ***) + vdim * nplanes * sizeof(void **) + vdim * nplanes * nrows * sizeof(void *));
...
inrimage *createInrimage(int ncols, int nrows, int nplanes, int vdim,
  ...
  MALLOC("Zinrimage:createInrimage",inr->data, void*, size );
  inr->array = index3DArray(inr->data, inr->ncols, inr->nrows, inr->nplanes,
			    sizeofImageVoxel(inr));
  if (inr->data == NULL) return NULL;
  inr->varray = NULL;

  //  for(i=0;i<size;i++)
  //  ((char*)(inr->data))[i] = 0;
  //  bzero( (void*) (inr->data), size);
...
inrimage *createNonInterlacedInrimage(int ncols, int nrows, int nplanes,
  ...
  if(vdim == 1) return createInrimage(ncols, nrows, nplanes, vdim, type);
    ...
    MALLOC("Zinrimage:createNonInterlaceInrimage", inr->data, void*, size);
...
void freeInrimage(inrimage *inr) {
  if(inr->data)  { free(inr->data);  inr->data = NULL; }
  if(inr->array) { free(inr->array); inr->array = NULL; }
  if(inr->varray){ free(inr->varray); inr->varray = NULL; }
  free(inr);
  inr = NULL;
}
