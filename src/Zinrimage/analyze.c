...
int readAnalyzeHeader( _image* im, const char* name )
{
   ...
   if(im->openMode != OM_CLOSE) {
      ...
      fread( &analyzeHeader, 1, sizeof(struct dsr), im->fd );
	 ...
	 char* data_filename = (char*) malloc(length+1) ;
...
writeAnalyzeHeader( const _image* im )
{
   ...
   hdr.dime.glmax  = 0.0 ;  /* maximum voxel value  */
   hdr.dime.glmin  = 0.0 ;  /* minimum voxel value */
...
/*     Calibration maximum and minimum values;  
    ...
    if(fwrite(&hdr,sizeof(struct dsr),1,im->fd) != 1)
       return -1;
...
int writeAnalyzeData(const _image *im) {
  if(im->openMode != OM_CLOSE) {
     ...
     char* buf = (char*) data + lineSize * im->ydim * im->zdim - lineSize ;
     unsigned LONG nwrt ;
     
     while( buf >= data )
     {
	nwrt = fwrite(buf, 1, lineSize, im->fd);
