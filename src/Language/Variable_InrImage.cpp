                                            ...
                                            )
{
  // process each component ???
  if ((!im1.get())||(!im2.get())) {
    GB_driver.err_print((boost::format("Empty input image smart pointer.") ).str()); 
    return Variable<InrImage>::ptr(); 
  }
  ...
  if (im1->GetVDim() == im2->GetVDim()) { 
    ...
    GB_driver.err_print((boost::format("Both images should have the same number of components for operator %1%.") % name ).str()); 
    return Variable<InrImage>::ptr(); 
}
